...
<Tabs>
  ...
  <Tab title="Via Helper Scripts">
    Some helper scripts may be available for automating Turnkey deployments:
    ```bash
    bash -c "$(wget -qLO - https://github.com/tteck/Proxmox/raw/main/ct/turnkey-app.sh)"
    ```

    **Note**: Not all Turnkey appliances have dedicated helper scripts. Use Proxmox native template system for most deployments.
  </Tab>
</Tabs>
